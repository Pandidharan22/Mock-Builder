...
- Define the chosen entity: for EVERY role present in ITS sample record you MUST define exactly one entity field — give it a camelCase name that matches the role's meaning (title, domain, score, age, commentCount, author, price, imageUrl, ...), choose the correct `type`, set a `uiHint`, and set `sourceRole` (see next rule). Do NOT drop roles — a thin entity that omits domain/age/score is wrong even though it may still validate.
- SOURCE ROLE (per field): every field MUST carry `sourceRole` = the EXACT role string of the record leaf this field is derived from. The available roles are exactly those shown in the representative record you were given (e.g. title, domain, score, age, comment_count, meta, rank, image, price, number). `sourceRole` names WHERE THE DATA COMES FROM (the record's role); it is INDEPENDENT of the field's `name`, which is the camelCase label YOU choose. Use the role string EXACTLY as given — do NOT invent role names, do NOT rename. Example: a field you name `author` that you read from a `meta` leaf has `sourceRole: "meta"` (NOT "author"); a field named `commentCount` read from a `comment_count` leaf has `sourceRole: "comment_count"`.
